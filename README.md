...
一个面向中文用户的 AI 情报导航站：把产品榜、KOL 信号和 AI 工具导购整理成清晰、可核验、可降级的发现系统。

## 当前完成度

- `/`：全新产品总览页，提供榜单、工具库和 KOL 三条发现路径
- `/rank/[type]`：5 个榜单，支持搜索、排序、响应式分页和数据来源说明
- `/library`：104 个精选工具、13 个分类，支持人群/分类/关键词筛选和增量加载
- `/library/[id]`：104 个静态详情页，包含适合人群、上手难度、使用场景、替代工具和判断信号
- 深浅主题、键盘焦点、跳转主内容、移动端 44px 触控目标和 `prefers-reduced-motion`
- Prisma 榜单批次存储、5 个抓取器、Vercel Cron、失败降级与数据库短时熔断
...
components/          布局、榜单、工具库和基础 UI
lib/library/         104 个工具、中文导购字段与精选榜算法
